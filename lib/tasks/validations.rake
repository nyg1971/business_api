...
namespace :validations do
  ...
  task validate_yaml_syntax: :environment do
    ...
    validation_files = Dir[Rails.root.join('config', 'validations', '*.yml')]
    errors = []
  ...
  desc "包括的チェック"
  task validate_all: [:validate_yaml_syntax, :validate_consistency] do
    puts "\n🎉 全ての検証が完了しました！"
  end
  ...
  def find_models_with_enums
    ...
    model_files.each do |file_path|
      # ファイル名からクラス名を推測
      relative_path = file_path.sub("#{Rails.root}/app/models/", '').sub('.rb', '')
